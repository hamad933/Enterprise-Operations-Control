...
|---|---|---|
| Schema | valid read, unknown field, malformed SHA, missing session ID, wrong repository | mutation/publication action-specific fields |
...
| Jules | no mutation API surface | simulated malformed/timeout/rate-limit fixtures, live read canary |
...
| Recovery | mutation absent/kill-by-design | interrupted mutation, StateStore recovery, publication recovery |
