...
|---|---|---|
| Schema | valid read, source inspection, alphanumeric session resource segment, slash rejection, unknown/irrelevant fields, malformed SHA, missing session ID, wrong repository | mutation/publication action-specific fields |
...
| Jules read safety | no mutation API surface, source/session/activity pagination, source/session inventory truncation, repeated page token, malformed provider JSON, resource-name mismatch, malformed/cross-session activity identity | timeout/rate-limit fixtures, live read canary, provider source-binding proof |
